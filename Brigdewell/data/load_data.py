...
def get_connection():
    print("PG_HOST =", os.getenv("PG_HOST", "localhost"))
    print("PG_PORT =", os.getenv("PG_PORT", 5432))
    print("PG_DB   =", os.getenv("PG_DB", "postgres"))
    print("PG_USER =", os.getenv("PG_USER", "postgres"))

    conn = psycopg2.connect(
        host=os.getenv("PG_HOST", "localhost"),
        port=os.getenv("PG_PORT", 5432),
        dbname=os.getenv("PG_DB", "mental_health_db"),
        user=os.getenv("PG_USER", "postgres"),
        password=os.getenv("PG_PASSWORD", "dianafreeman")
    )
...
if __name__ == "__main__":
    csv_path = sys.argv[1] if len(sys.argv) > 1 else "mental_health.csv"
    load(csv_path)